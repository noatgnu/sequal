...
osequon = Modification(
    "Mannose", regex_pattern="[S|T]", mod_type="variable", labile=True, mass=162.05
)
sulfation = Modification(
    "Sulfation", regex_pattern="S", mod_type="variable", labile=True, mass=79.95680
)
carbox = Modification(
    "Carboxylation", regex_pattern="E", mod_type="variable", labile=True, mass=43.98983
)
propiona = Modification(
    "Propionamide", regex_pattern="C", mod_type="static", mass=71.037114
)
    ...
    def test_fragment_mass_calculation(self):
        ...
        for i in g.generate():
            ...
            for b, y in fragment_non_labile(seqs, "by"):
                ...
                mass_b = calculate_mass(
                    b, N_terminus=1.007825, O_terminus=0, with_water=False
                )
                print(mass_b)
                print(y, "y{}".format(y.fragment_number))
                mass_y = calculate_mass(
                    y, N_terminus=0, O_terminus=15.99491463 + 1.007825, with_water=False
                )
                print(mass_y)
